...
Baseline repo ini dibuat untuk lab HA 5 node: **3 controller**, **2 compute**, 1 deployer di luar cluster, dan backend storage memakai **external Ceph RBD**.
...
> Catatan: playbook `00-preflight.yml` saat ini memang memvalidasi Ubuntu **22.04**. Kalau dipakai untuk OS lain, validasi preflight dan nama paket service perlu dicek ulang.

---

## Topologi Lab

Baseline lab yang dipakai repo ini:
...
Default interface pada inventory lab:
...
## Yang Wajib Disesuaikan Sebelum Dipakai di Lab Lain atau Production
...
Beberapa konfigurasi di repo ini sengaja dibuat nyaman untuk lab. Untuk production, minimal lakukan hal berikut:
...
- Jangan commit `.vault_pass`, `vault.yml` asli, `ceph.client.*.keyring`, atau file rahasia lain ke repository publik.
...
Atau jalankan validator bawaan:
